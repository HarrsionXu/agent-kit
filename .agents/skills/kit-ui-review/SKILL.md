---
name: kit-ui-review
description: 对照项目设计基线验证前端视觉、交互和工程质量。适用于 Angular、Vue 和 React 页面的交付验收、UI 审查或视觉回归；新页面的设计阶段使用 kit-ui-design。
---

# UI / UX 验收

阅读当前范围适用的 frontend 与 frontend-design 规则、项目设计基线、共享组件及相关认可样板；没有样板则说明缺口，不强行凑数或将任意历史页面认定为标准。审查请求默认只读；已获实现授权时修正范围内问题并重验。不要从通用设计趋势列表中另选一套视觉风格。
...
ui-ux-pro-max 等外部 Skill 仅在具体问题需要补充参考或用户指定时使用，不因 UI 验收自动检索或生成新设计系统。实际使用后说明采纳依据；不能仅因已安装就宣称使用过。不得擅自下载其他 Skill，也不得擅自向外部设计服务发送项目私有信息。

交付按验收项记录预期/实际结果及证据位置，注明基线、路由、视口与数据状态；分别说明自动检查、浏览器交互及尚未验证的部分。信息层级和操作效率需结合真实任务判断，机械扫描通过不等于设计合格。
